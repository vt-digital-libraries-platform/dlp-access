...
function parseTypeFields(typeName) {
  ...
  const typeFields = schemaText.substring(openBraceIndex + 1, closeBraceIndex - 1);
  ...
  while ((match = fieldRegex.exec(typeFields)) !== null) {
    ...
    if (fieldName === 'allow' || fieldName === 'operations' || fieldName === 'groups' || fieldName === 'groupsField') {
      continue;
    }
...
export function generateMetadataFields() {
  ...
  // Accessibility fields not yet in schema but should be documented
  const accessibilityFields = ['visualDescription', 'alt', 'archived'];
  ...
  archiveFieldNames.forEach(fieldName => {
    ...
    const defaultType = formatTypeString(schemaInfo.graphqlType, schemaInfo.isRepeatable);
    ...
    const labelName = displayInfo?.labelName || fieldName.split('_').map(word => 
  ...
  collectionFieldNames.forEach(fieldName => {
    ...
    const description = displayInfo?.description || `${labelName} field (Collection-level only)`;
    const example = displayInfo?.example || '';
  ...
  // Add accessibility fields (not yet in schema but documented for future implementation)
  accessibilityFields.forEach(fieldName => {
    const displayInfo = metadataFieldInfo[fieldName];
    if (displayInfo) {
      metadataFields.push({
        columnName: fieldName,
        labelName: displayInfo.labelName,
        type: fieldName === 'visualDescription' ? 'JSON (Dictionary)' : 
              fieldName === 'alt' ? 'String' : 'Boolean',
